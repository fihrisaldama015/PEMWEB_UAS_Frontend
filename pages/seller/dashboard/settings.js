  ...
  const updateProfile = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/seller/v1/shop/data/${dataToko.toko_id}`,
        formdata,
  ...
  const getDataToko = async () => {
    ...
    try {
      ...
      const {
        ...
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/seller/v1/shop/data/${toko_id}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setDataToko(data);
      ...
      const {
        ...
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/seller/v1/shop/image/${data.image_profile}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setTokoPic({ filename });
      setIsLoading(false);
        ...
        <div className="p-3">
          <div className="p-3 flex flex-col justify-between items-center transition-all">
            ...
            {!file ? (
              !isLoading ? (
                <img
                  src={`${process.env.NEXT_PUBLIC_BASE_URL}/api/seller/file/toko/${tokoPic.filename}`}
