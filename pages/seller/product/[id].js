  ...
  const getDetailProduct = async () => {
    try {
      const {
        ...
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/seller/v1/product/data/${id}`
      );
  ...
  const getTokoId = async () => {
    ...
    try {
      const {
        ...
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/seller/v1/shop/data/${tokoId}`,
  ...
  const editProduct = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `${process.env.NEXT_PUBLIC_BASE_URL}/api/seller/v1/product/data/${toko_id}/${id}`,
        formdata,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      {files ? (
        ...
        <div className="p-3 grid grid-cols-3 gap-3 rounded-xl bg-mygreen_dark/10">
          {filename.map((name, id) => (
            <Image
              key={id}
              src={`${process.env.NEXT_PUBLIC_BASE_URL}/api/seller/file/product/${name}`}
              height={100}
              width={100}
              priority
              className="rounded-xl"
